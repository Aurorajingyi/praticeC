...
int main()
{
	struct point y={0,0}; //创建了一个“point”结构类型的变量y，并初始化 
	getStruct(&y); //调用 getStruct 函数，将 y 结构体的地址传递给它
	output(y);//将 y 结构体作为参数传递给它。
	try(*getStruct(&y));//getStruct(&y)返回的是指针p，*getStruct(&y)是p所指的变量 
	print(getStruct(&y)); //getStruct得到的指针传给print 
}
...
//跟getStruct是类似的，区别是这里是只读的，不能对p进行修改 
void print(const struct point *p)
{
	printf("print:%d,%d\n",p->x,p->y);
}
